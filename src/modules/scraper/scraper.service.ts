...
export class ScraperService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.scrapeIkigai(1, 30).catch(err => this.logger.error(`Initial Ikigai scrape failed: ${err}`));
    this.scrapeOlympus(1, 20).catch(err => this.logger.error(`Initial Olympus scrape failed: ${err}`));
